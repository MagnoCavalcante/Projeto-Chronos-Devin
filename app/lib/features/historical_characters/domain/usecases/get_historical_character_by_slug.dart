...
/// Caso de Uso (Use Case) responsável por orquestrar a recuperação de um Personagem Histórico publicado e ativo por slug.
///
...
class GetHistoricalCharacterBySlug {
  final HistoricalCharacterRepository _repository;
  static const String _tag = 'GetHistoricalCharacterBySlug';
  ...
  /// Executa o caso de uso de negócio para recuperar o Personagem Histórico do CHRONOS pelo slug.
  ///
  /// Retorna um [Result] contendo o [HistoricalCharacter] em caso de sucesso,
  /// ou uma falha pura de negócio [Failure] em caso de erro.
  Future<Result<HistoricalCharacter>> call(String slug) async {
    final stopwatch = Stopwatch()..start();
    ChronosLogger.info(
      'Iniciando a execução do caso de uso obter Personagem Histórico por slug: $slug...',
      tag: _tag,
    );

    final result = await _repository.getCharacterBySlug(slug);
    stopwatch.stop();

    if (result.isSuccess) {
      ChronosLogger.info(
        'Caso de uso de obter Personagem Histórico por slug concluído com sucesso! | Tempo: ${stopwatch.elapsedMilliseconds}ms',
        tag: _tag,
      );
    } else {
      final failure = result.failureOrNull;
      ChronosLogger.error(
        'Falha ocorrida durante a execução do caso de uso obter Personagem por slug: ${failure?.message} | Tempo: ${stopwatch.elapsedMilliseconds}ms',
        tag: _tag,
